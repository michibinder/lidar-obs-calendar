...
// TMP or BWF plot state - check which plot has to be visualized
// check if change is active (button was pressed), could be combined with nightly mean
...
if (isset($_GET['content_state'])) {
    if ($_GET['content_state'] == '1'){
        if ($content_change == 0){$content_state = 1;}
        else {$content_state = 0;}
    } else {
        if ($content_change == 1){$content_state = 1;}
        else {$content_state = 0;}
    }
}
...
// Determine the required state for Button and nightly means image
if (isset($_GET['nm_state'])) {
    if ($_GET['nm_state'] == '1'){
        if ($nm_change == 0){$nm_state = 1;}
        else {$nm_state = 0;}
    } else {
        if ($nm_change == 1){$nm_state = 1;}
        else {$nm_state = 0;}
    }
}

// Only $nm_state is now relevant
if ($nm_state == 0) {
    $nm_div = 'hide_nm';
...
// Only $content_state is now relevant
if ($content_state == 0) {
    $toggle_button_bwf = $toggle_off;
} else {
    $toggle_button_bwf = $toggle_on;
}
        ...
        <div class="col-xl-7 col-lg-8 col-md-12"> 
            <div class="container">
                ...
                <p>
                    ...
                    <span><a href="?ym=<?= $ym; ?>&index=<?= $i_plot; ?>&nm_state=<?= $nm_state; ?>&content_state=<?= $content_state; ?>&change=1" class="btn btn-dark">Nightly means overview <?php echo $toggle_button ?></a></span>
                    <span><a href="?ym=<?= $ym; ?>&index=<?= $i_plot; ?>&nm_state=<?= $nm_state; ?>&content_state=<?= $content_state; ?>&change=2" class="btn btn-dark">BW filter <?php echo $toggle_button_bwf ?></a></span>
...
<p>
                <div class="container">
                        <label class="form-control">   
                            <input type="radio" name="radio" <?php if ($content_state==0) {echo 'checked';} else {echo '';} ?>/>
                    ...
                    <label class="form-control">
                        <input type="radio" name="radio" <?php if ($content_state==1) {echo 'checked';} else {echo '';} ?>/>
                            <a href="?ym=<?= $ym; ?>&index=<?= $i_plot; ?>&nm_state=<?= $nm_state; ?>&content_state=1&change=2">
                            Vertical butterworth filter       
                            </a>
                    </label>
                    
                    <label class="form-control">
                        <input type="radio" name="radio" <?php if ($content_state==2) {echo 'checked';} else {echo '';} ?>/>
                            <a href="?ym=<?= $ym; ?>&index=<?= $i_plot; ?>&nm_state=<?= $nm_state; ?>&content_state=2&change=2">
                            Temporal filter (nightly mean)
                            </a>
                    </label>
                </div>
                </p>
